...
def run():
    ...
    meta = {'ok': True, 'generatedAt': datetime.now(timezone.utc).replace(microsecond=0).isoformat(), 'total': len(all_items), 'window': WINDOW, 'minScore': MIN_SCORE, 'byCategory': dict(Counter(i['category'] for i in all_items)), 'errors': errors}
