...
function App() {
  ...
  return <main className="wrap">
    <header className="hero">
      <div>
        <p className="kicker">GitHub Pages · auto-updating</p>
        <h1>AI Harms Monitor</h1>
        <p>Horizon scanning mapped to AI harm categories, with UK boost, relevance scoring and safer link handling.</p>
      </div>
    ...
    <section className="stats">
      ...
      <div><span>Updated</span><b>{fmtDate(meta.generatedAt)}</b></div>
    </section>
    ...
    <section className="filters">
      ...
      <label><input type="checkbox" checked={safeLinksOnly} onChange={e => setSafeLinksOnly(e.target.checked)} /> safe links only</label>
    ...
    {meta.errors && Object.keys(meta.errors).length > 0 && <p className="notice">Some sources failed. Dashboard still loaded with available results.</p>}
    ...
    <section className="list">
      {filtered.map((a, i) => <article className="item" key={a.id || a.url || i}>
        ...
        {a.url && a.link_safe ? <a className="open" href={a.url} target="_blank" rel="noreferrer">Open</a> : <span className="blocked">No direct link</span>}
      </article>)}
      {!news.loading && filtered.length === 0 && <p className="notice">No results match these filters. Try lowering min score, clearing filters, or rerunning the workflow.</p>}
    </section>
  </main>;
}
